...
- [ ] **Task 1: Update Database Schema** (AC: All)
  - [ ] 1.1 Add migration: `parsed_sections` column (JSONB, nullable)
  - [ ] 1.2 Add migration: `parsing_status` column (TEXT, default 'pending', values: pending/completed/failed)
  - [ ] 1.3 Add migration: `parsing_error` column (TEXT, nullable)
  - [ ] 1.4 Add index on `parsing_status` for querying pending parses

- [ ] **Task 2: Define Section Parser Data Structures** (AC: All)
  - [ ] 2.1 Define TypeScript types in `lib/parsers/types.ts`:
  ...
  - [ ] 2.2 Define Zod schema for validation of parsed output

- [ ] **Task 3: Create Section Header Detection** (AC: 1, 5)
  - [ ] 3.1 Create `lib/parsers/resume.ts` with `detectSections(text: string): Map<string, string>`
  - [ ] 3.2 Implement regex patterns for common section headers:
  ...
  - [ ] 3.4 Handle variations (multiple words, with/without colon)
  - [ ] 3.5 Return sections in order found, with start/end line numbers

- [ ] **Task 4: Implement Experience Parser** (AC: 2)
  - [ ] 4.1 Create `parseExperienceSection(text: string): JobEntry[]`
  - [ ] 4.2 Detect job entries (separated by dates, company names, or line breaks)
  - [ ] 4.3 Extract: company name, job title, dates (various formats)
  - [ ] 4.4 Extract bullet points (lines starting with -, •, *, etc.)
  - [ ] 4.5 Return structured JobEntry array
  - [ ] 4.6 Handle edge cases: missing dates, multiple titles at same company

- [ ] **Task 5: Implement Education Parser** (AC: 3)
  - [ ] 5.1 Create `parseEducationSection(text: string): EducationEntry[]`
  - [ ] 5.2 Detect education entries (typically one per line or separated by blank lines)
  - [ ] 5.3 Extract: institution, degree, dates, GPA (if present)
  - [ ] 5.4 Handle variations: "B.S. Computer Science", "BS in CS", "Bachelor of Science"
  - [ ] 5.5 Handle GPA formats: "3.8", "3.8 GPA", "GPA: 3.8/4.0"
  - [ ] 5.6 Return structured EducationEntry array

- [ ] **Task 6: Implement Skills Parser** (AC: 4)
  - [ ] 6.1 Create `parseSkillsSection(text: string): Skill[]`
  - [ ] 6.2 Split skills by comma, semicolon, or line breaks
  - [ ] 6.3 Categorize as technical vs soft:
  ...
  - [ ] 6.4 Use keyword lists or heuristics for categorization
  - [ ] 6.5 Return Skill array with category
  - [ ] 6.6 Handle grouped skills: "Languages: Python, Java" → extract and categorize

- [ ] **Task 7: Implement Main Parsing Orchestrator** (AC: 1, 5, 6)
  - [ ] 7.1 Create `parseResumeText(text: string): ParsedResume`
  - [ ] 7.2 Call section detection to identify all sections
  - [ ] 7.3 For each identified section, call appropriate parser
  - [ ] 7.4 For unknown sections, extract raw text and put in "Other"
  - [ ] 7.5 Handle parsing errors gracefully (don't fail on bad data)
  - [ ] 7.6 Return structured ParsedResume object

- [ ] **Task 8: Extend uploadResume + extractionAction** (AC: All)
  - [ ] 8.1 Create new action: `parseResumeSection(resumeId: string): Promise<ActionResponse<ParsedResume>>`
  - [ ] 8.2 Modify uploadResume workflow:
  ...
  - [ ] 8.3 Handle parsing errors gracefully (non-blocking)
  - [ ] 8.4 Update resume record with results

- [ ] **Task 9: Error Handling & Validation** (AC: All)
  - [ ] 9.1 Validate parsed output against Zod schema
  - [ ] 9.2 On validation error, set parsing_status to 'failed' with error message
  - [ ] 9.3 Log parsing errors for debugging
  - [ ] 9.4 Return parsing_status and parsed_sections (even if partial)

- [ ] **Task 10: Create E2E Tests** (AC: 1-6)
  - [ ] 10.1 Create `tests/e2e/resume-parsing.spec.ts`
  - [ ] 10.2 Test AC1: Basic section categorization with sample resumes
  - [ ] 10.3 Test AC2: Experience section parsing (job entries, dates, titles)
  - [ ] 10.4 Test AC3: Education section parsing (institutions, degrees, dates)
  - [ ] 10.5 Test AC4: Skills section parsing (technical vs soft skills)
  - [ ] 10.6 Test AC5: Non-standard sections in "Other"
  - [ ] 10.7 Test AC6: Parsed data stored in database as JSON
...
- [ ] **Task 11: Final Verification** (AC: 1-6)
  ...
  - [ ] 11.3 Verify E2E tests pass (requires dev server)
  - [ ] 11.4 Manual test: Upload resume → extraction → parsing → verify DB contains parsed_sections
  - [ ] 11.5 Manual test: Verify parsed sections are properly structured as JSON
  - [ ] 11.6 Manual test: Test with various resume formats
...
1. **Define types** (lib/parsers/types.ts)
2. **Create section detection** (header regex patterns)
3. **Implement experience parser** (job entry extraction)
4. **Implement education parser** (education entry extraction)
5. **Implement skills parser** (technical/soft categorization)
6. **Create orchestrator** (main parsing function)
7. **Extend server action** (trigger parsing after extraction)
8. **Write E2E tests** (comprehensive parsing scenarios)
9. **Final verification** (build, lint, tests)

### File List

**Will be created during dev-story:**
- `lib/parsers/types.ts` - TypeScript types for parsed resume
- `lib/parsers/experience.ts` - Experience section parser
- `lib/parsers/education.ts` - Education section parser
- `lib/parsers/skills.ts` - Skills section parser
- `tests/e2e/resume-parsing.spec.ts` - E2E tests
- `supabase/migrations/005_add_parsing_columns.sql` - Database schema

**Will be modified:**
- `actions/resume.ts` - Add parseResumeSection action
- `lib/parsers/resume.ts` - Add orchestrator function
- `lib/validations/resume.ts` - Add parsing schema
- `_bmad-output/implementation-artifacts/sprint-status.yaml` - Mark ready-for-dev

**No changes:**
- `components/forms/ResumeUpload.tsx` - Unchanged
- `app/(dashboard)/scan/new/page.tsx` - Unchanged
