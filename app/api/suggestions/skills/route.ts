...
/**
 * Build ATS context for skills section if available
 */
async function buildATSContextForSection(
  ...
  try {
    ...
    if (!sessionResult || sessionResult.error || !sessionResult.data) {
      console.log('[SS:skills] Could not fetch session for ATS context');
      return undefined;
    }
    ...
    if (!atsScore || !keywordAnalysis) {
      console.log('[SS:skills] No ATS score or keyword analysis in session');
      return undefined;
    }

    if (!isATSScoreV21(atsScore)) {
      console.log('[SS:skills] ATS score is not V21 format, skipping context');
      return undefined;
    }

    console.log('[SS:skills] Building ATS context for skills section');
    return buildSectionATSContext('skills', {
      atsScore,
      keywordAnalysis,
      resumeText,
    });
  } catch (error) {
    console.error('[SS:skills] Error building ATS context:', error);
    return undefined;
  }
}
